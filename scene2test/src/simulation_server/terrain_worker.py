"""Isolated terrain-course rollout. Not dispatched by the legacy navigation server."""
...
def run_terrain(request, output: Path, groot_root: Path, bundle: Path):
    ...
    from .worlds import compose_model, validate_bundle

    spec = validate_bundle(bundle, request.resources.scene.revision)
    if not hasattr(spec, "surfaces"):
        raise ValueError("terrain runner requires terrain-course-v1")
    from procedural_world.terrain import ground_height

    from .terrain_metrics import TerrainMetrics

    nav = navigation_map(spec)
    model, source_hashes = compose_model(spec, groot_root, output)
    terrain_metrics = TerrainMetrics(spec, model)
    ...
    world_ids |= terrain_metrics.support_ids
            ...
            for _ in range(math.ceil(request.execution.maximum_duration_s / model.opt.timestep)):
                ...
                for c in data.contact:
                    if c.dist <= 0 and (
                        ...
                        or terrain_metrics.nonfoot_collision(c)
                    ):
                        collision = True
                        contacts.write(
                            json.dumps(
                ...
                clearance = terrain_metrics.update(data)
                fallen = clearance < 0.45 or max(abs(roll), abs(pitch)) > 1.0
    ...
    facts.update(terrain_metrics.result())
    facts["ground_height_at_end_m"] = ground_height(spec, data.qpos[:2])
    reproduction = {
        ...
        "navigator": {
            "id": "gt-waypoint-terrain-v1",
            "base_navigator": VERSION,
            "parameters": params,
        ...
        "metrics_version": "navigation-terrain-1.0",
        "task_facts": facts,
    }
    from procedural_world import terrain as terrain_module

    for p in (
        Path(terrain_module.__file__),
        Path(__file__).with_name("path_tracking.py"),
        Path(__file__).with_name("terrain_metrics.py"),
    ):
        reproduction["code_sha256"][p.name] = hashlib.sha256(p.read_bytes()).hexdigest()
...
def _plot(spec, nav, trajectory, path):
    ...
    ax.plot(reference[:, 0], reference[:, 1], "--", label="Reference 2.5D planned path")
    if trajectory:
