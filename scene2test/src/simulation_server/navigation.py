...
def simplify_path(nav):
    ...
    while index < len(points) - 1:
        ...
        for j in range(len(points) - 1, index, -1):
            if segment_free(points[index], points[j], nav):
                next_index = j
                break
...
class Follower:
    def __init__(self, nav, speed=0.3, tolerance=0.25, boxes=None):
        self.nav = nav
        self.boxes = boxes
        self.path = simplify_path(nav)
        self.index = 1
        self.speed = speed
