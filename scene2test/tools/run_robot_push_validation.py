...
def main():
    ...
    p.add_argument("--distance", type=float, default=0.08)
    ...
    p.add_argument(
        "--output-root",
        type=Path,
        default=Path("/workspace/g1_failure/runtime/robot_push_validation"),
    )
    args = p.parse_args()
    ...
    if not 0.075 <= args.distance <= 0.20:
        p.error("distance 0.075..0.20 m required")
    os.environ.setdefault("MUJOCO_GL", "egl")
    os.environ["SIM_SERVER_ONNX_PROVIDER"] = "cuda"
    import mujoco

    from clear_path import audit, contact_control, fixture, push_probe
    from clear_path.contracts import Fixture
    from robot_vlm import push_skill
    from robot_vlm import push_validation as contact_probe
    from simulation_server import groot_locomotion

    config = Fixture(box_mass_kg=args.mass, box_friction=args.friction)
    ...
    try:
        ...
        protocol = {
            ...
            "kind": args.kind,
            "fixture_config": config.model_dump(),
            "requested_distance_m": args.distance,
            "maximum_duration_s": duration,
            "physics_dt": model.opt.timestep,
            "fixture_revision": fixture.identity(config),
            "mujoco_version": mujoco.__version__,
            "initial_robot_y": args.initial_y,
            "controller_condition": push_skill.VERSION,
