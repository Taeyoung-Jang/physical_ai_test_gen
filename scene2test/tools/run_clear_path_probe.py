...
def main():
    ...
    p.add_argument("--kind", choices=["arm_reach", "push_contact"], default="arm_reach")
    ...
    args = p.parse_args()
    duration = args.duration if args.duration is not None else 8 if args.kind == "arm_reach" else 11
    if not 4 <= duration <= 20:
        p.error("duration must be 4..20 seconds")
    os.environ.setdefault("MUJOCO_GL", "egl")
    os.environ["SIM_SERVER_ONNX_PROVIDER"] = "cuda"
    import mujoco

    from clear_path import audit, fixture, push_probe
    ...
    try:
        ...
        protocol = {
            ...
            "code_sha256": {
                ...
                for m in (audit, fixture, push_probe, groot_locomotion)
            },
            "cli_sha256": audit.sha256(Path(__file__)),
        }
        write(root / "protocol.json", protocol)
        result = push_probe.run_probe(
            model, args.groot_root, root, kind=args.kind, duration=duration
        )
...
손 접촉 step: {result["hand_contact_steps"]}.</p>
