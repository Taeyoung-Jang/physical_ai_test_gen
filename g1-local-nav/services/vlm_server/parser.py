"""Strict parser (blueprint §11.2) — forces raw VLM output into exactly one of the allowed
action tokens, or STOP on any ambiguity/failure. GRASP/RELEASE (pick-and-carry extension beyond
blueprint §14) were added alongside the original 4 locomotion tokens once
scripts/run_pickup_smoke.py proved the underlying weld-based grasp mechanism live.

No imports from src/g1_local_nav here on purpose: the VLM server and the sim/control process
are meant to be fully decoupled (blueprint §5.1, "의존성 충돌과 장애 격리"), so the valid
...
VALID_ACTIONS = ("FORWARD", "BACKWARD", "TURN_LEFT", "TURN_RIGHT", "STOP", "GRASP", "RELEASE")
...
def parse_action(raw_text: str) -> tuple[str, bool]:
    ...
    Rules (blueprint §11.2):
      ...
      4. If exactly one of VALID_ACTIONS appears as a whole word anywhere, use it.
